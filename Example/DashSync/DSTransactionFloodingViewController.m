...
- (void)confirmTransaction:(DSTransaction *)tx toAddress:(NSString *)address forAmount:(uint64_t)amount {
    ...
    if (!tx) { // tx is nil if there were insufficient wallet funds
        if (authenticationManager.didAuthenticate) {
            ...
            [authenticationManager seedWithPrompt:@"seed"
                                        forWallet:self.fundingAccount.wallet
                                        forAmount:amount
        ...
        [self.fundingAccount signTransaction:tx
                                  withPrompt:nil
                                  completion:^(BOOL signedTransaction, BOOL cancelled) {
                                      if (!signedTransaction) {
                                          ...
                                          [self.chainManager.transactionManager publishTransaction:tx
